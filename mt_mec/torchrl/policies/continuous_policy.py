...
import numpy as np
import mt_mec.torchrl.networks.nets as networks_nets
import mt_mec.torchrl.networks.init as networks_init
...
class DetContPolicy(networks_nets.Net):
    def forward(self, x):
...
class FixGuassianContPolicy(networks_nets.Net):
...
class GuassianContPolicy(networks_nets.Net):
    def forward(self, x):
        x = super().forward(x)
...
class GuassianContPolicyBasicBias(networks_nets.Net):
...
class ModularGuassianGatedCascadeCondContPolicy(networks_nets.ModularGatedCascadeCondNet, EmbeddingGuassianContPolicyBase):
    ...
    def forward(self, x, embedding_input, return_weights = False ):
        ...
        mean, log_std = x.chunk(2, dim=-1)  # 公式9的最后步骤
...
class MultiHeadGuassianContPolicy(networks_nets.BootstrappedNet):
