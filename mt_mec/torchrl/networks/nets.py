...
import mt_mec.torchrl.networks.init as init
...
class ModularGatedCascadeCondNet(nn.Module):
    def __init__(self, output_shape,
        ...
        self.dropout_modules = nn.Dropout(p=0.5)
        self.dropout_weights1 = nn.Dropout(p=0.05)
        self.dropout_weights2 = nn.Dropout(p=0.05)
        self.dropout_weights3 = nn.Dropout(p=0.05)
        self.dropout_weights_list = [self.dropout_weights1, self.dropout_weights2, self.dropout_weights3]

        for i in range(num_layers):  # base policy network部分
            layer_module = []
            for j in range( num_modules ):
                # print("i:{}, j:{},module_input_shape:{},module_hidden:{}".format(i, j, module_input_shape, module_hidden))
        ...
        self.gating_fcs = []
        for i in range(num_gating_layers):  # routing network部分
    ...
    def forward(self, x, embedding_input, return_weights = False):
        ...
        for gating_weight_fc, gating_weight_cond_fc in zip(self.gating_weight_fcs, self.gating_weight_cond_fcs):
            ...
            cond = cond * embedding
            cond = self.activation_func(cond)  # 类似公式6的实现

            raw_weight = gating_weight_fc(cond)
            raw_weight = raw_weight.view(weight_shape)
            softmax_weight = F.softmax(raw_weight, dim=-1)  # 类似公式7的实现
            weights.append(softmax_weight)
            if self.pre_softmax:
                flatten_weights.append(raw_weight.view(flatten_shape))
            else:
        ...
        # print(">>>>>>>>>>>>>>>>layers:{},modules:{}".format(self.num_layers - 1, len(self.layer_modules[0 + 1])))
        weights_array_for_vis = None
        for i in range(self.num_layers - 1):
            new_module_outputs = []
            for j, layer_module in enumerate(self.layer_modules[i + 1]):
                # weights[i] = self.dropout_weights_list[i](weights[i])
                module_input = (module_outputs * \
                    weights[i][..., j, :].unsqueeze(-1)).sum(dim=-2)
                # print("===module_outputs:", module_outputs.shape)
                # print("===weight:", weights[0].shape)
                # module_input = self.dropout_modules(module_input)
                module_input = self.activation_func(module_input)  # 类似公式8的实现
                new_module_outputs.append((
                        layer_module(module_input)
                ).unsqueeze(-2))

            module_outputs = torch.cat(new_module_outputs, dim = -2)
            if weights_array_for_vis is None:
                weights_array_for_vis = weights[i]
            else:
                weights_array_for_vis = torch.cat((weights_array_for_vis, weights[i]), dim=0)

        # print("vis:", weights_array_for_vis)
