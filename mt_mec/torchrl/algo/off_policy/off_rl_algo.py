...
from mt_mec.torchrl.algo.rl_algo import RLAlgo
...
class OffRLAlgo(RLAlgo):
    ...
    def pretrain(self):
        ...
        for pretrain_epoch in range( self.pretrain_epochs ):
            ...
            training_epoch_info =  self.collector.train_one_epoch()  # collector训练一个回合
            for reward in training_epoch_info["train_rewards"]:  # 这里的‘train_rewards’对于MT10包含10个reward
            ...
            infos["Running_Training_Average_Rewards"] = np.mean(self.training_episode_rewards)  # 所有task的平均值
