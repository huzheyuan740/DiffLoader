...
class Logger():
    def __init__(self, experiment_id, env_name, seed, params, log_dir = "./log"):
        ...
        self.tf_writer = SummaryWriter(work_dir)
    ...
    def add_epoch_info(self, epoch_num, total_frames, total_time, infos, csv_write=True):
        ...
        for info in infos:
            if '_train_' in info:
                self.tf_writer.add_scalar("1_Train/"+info, infos[info], epoch_num)
            elif '_eval_' in info:
                self.tf_writer.add_scalar("2_Eval/"+info, infos[info], epoch_num)
            else:
                self.tf_writer.add_scalar("3_Rest/"+info, infos[info], epoch_num)
