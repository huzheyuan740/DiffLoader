...
from .d4rl import load_environment, sequence_dataset, load_dmc_dataset, load_metaworld_dataset, load_mec_dataset
...
class MetaSequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, replay_dir_list=[], task_list=[], horizon=64,
                 normalizer='LimitsNormalizer', preprocess_fns=[], max_path_length=1000,
                 max_n_episodes=200000, termination_penalty=0, use_padding=True, seed=None, meta_world=False,
                 maze2d=False, antmaze=False, optimal=True, global_config=None, env='hopper-medium-replay'):
        ...
        if meta_world:
            ...
            itr = load_mec_dataset(self.replay_dir_list, self.task_list, global_config)
        ...
        self.normalizer = DatasetNormalizer(fields, normalizer, path_lengths=fields['path_lengths'])
        self.indices = self.make_indices(fields.path_lengths, horizon)  # len(self.indices): 1872000=4000*(500-32)
    ...
    def make_indices(self, path_lengths, horizon):
        ...
        indices = []
        for i, path_length in enumerate(path_lengths):
            # print(f"I:<{i},{path_length}>{self.use_padding}")
            max_start = min(path_length - 1, self.max_path_length - horizon)
            if not self.use_padding:
                max_start = min(max_start, path_length - horizon)
            for start in range(max_start):
...
class RTGActMecDataset(MetaSequenceDataset):
    '''
        adds a value field to the datapoints for training the value function
    '''
    ...
    def __getitem__(self, idx, eps=1e-4):
        ...
        for i in range(len(self.task_list)):
            path_inds.append((path_ind+interval*i)%self.fields.n_episodes)
        # path_inds: [10, 10010, 20010]
        observations = np.zeros((len(path_inds), self.seq_length, self.observation_dim))
        count = start
        # print("path_inds:", path_inds)
        k = self.seq_length - 1
        # print("self.fields.normed_observations:", self.fields.normed_observations.shape)
        # print("self.fields.normed_actions:", self.fields.normed_actions.shape)
        while count >= 0 and k >= 0:
            # print("self.fields.observations:", self.fields.observations[path_inds, count])
            observations[:, k, :] = self.fields.normed_observations[path_inds, count]
            # print("observations:", np.around(self.normalizer.unnormalize(observations[:, k, :], 'observations'), 5))
            k -= 1
            count -= 1
        # print("origin_action:", self.fields.actions[path_inds, start:end].shape)
        actions = self.fields.normed_actions[path_inds, start:end]
        # print("actions:", np.around(self.normalizer.unnormalize(actions, 'actions'), 5))
        # print("diff:", np.max(self.fields.actions[path_inds, start:end] - self.normalizer.unnormalize(actions, 'actions')))
        task = np.array([self.task_list.index(self.fields.get_task(path_ind)) for path_ind in path_inds])#self.get_task_id(self.fields.get_task(path_ind))
        rtg = self.discount_cumsum(self.fields['rewards'][path_inds, start:], gamma=self.discount)[:,:end-start] / (self.max_path_length-start)
