...
def load_mec_dataset(replay_dir, task_list, global_config):
    data_ = collections.defaultdict(list)
    # save = [
    # replay_dir = '/NAS2020/Workspaces/DRLGroup/hrhe/metaworld.pytorch/logs'
    leng = global_config.train_config.episode_num  # TODO 暂定训练数据是所有回合
    for task_id in range(len(task_list)):  # loop
        _replay_dir = os.path.join(replay_dir, f'seed_{global_config.train_config.seed}', task_list[task_id])
        _replay_dir = Path(_replay_dir)
        eps_fns = sorted(_replay_dir.glob('*.npy'), key=lambda x: int(x.stem))
        for eps_fn in [eps_fns[i] for i in range(leng)]:  # eps_fns[-20:]:#x:#eps_fns[-20:]:
            episode = load_mec_episode(eps_fn)
            for i in range(episode["terminals"].shape[0]):
                done_bool = bool(episode['terminals'][i])
                for k in episode:
                    # print("k:{}, data:{}".format(k, episode[k].shape))
                    # if k == 'rewards':
                    #     print("episode[k]:", np.mean(episode[k]))
                    if k == 'task':
                        continue
                ...
                if done_bool:
                    ...
                    # print('episode_data:', episode_data['rewards'].shape)
                    # exit()
                    yield episode_data
                    data_ = collections.defaultdict(list)
...
def load_metaworld_dataset(replay_dir, task_list, optimal=True):
    ...
    for task_id in range(len(task_list)):  # loop
        ...
        eps_fns = sorted(_replay_dir.glob('*.npz'), key=lambda x: int(x.stem))
        for eps_fn in [eps_fns[i] for i in range(leng)]:  # eps_fns[-20:]:#x:#eps_fns[-20:]:
