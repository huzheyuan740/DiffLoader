...
def get_prompt(trajectories, num_episodes, num_steps, is_meta, is_maze=False):
    ...
    # print("trj_ids[i]:", trajectories[trj_ids[0]]['observations'].shape)
    for i in range(num_episodes):
        current_path_length = trajectories[trj_ids[0]]['observations'].shape[0]
        if start_steps[i] + num_steps <= current_path_length:
            continue
        else:
            # print("change!")
            start_steps[i] = current_path_length - num_steps
...
def get_prompt_for_language(trajectories, num_episodes, num_steps, is_meta, is_maze=False):
    trj_ids = np.random.choice(
        np.arange(len(trajectories)),
        size=num_episodes,
        replace=False,
...
def get_prompt_batchs(prompt_trajectories, cond, prompt_type=None, num_episodes=2, num_steps=20, is_meta=True):
    cond = cond.long()
    trajectories = [0]
    if prompt_type == 'one-hot':
        trajectories = [get_prompt_for_one_hot(prompt_trajectories[ind], ind, num_episodes, num_steps, is_meta) for ind
                        in cond]
...
def get_prompt_maze2d(prompt_trajectories, cond, num_episodes=1, num_steps=50, is_meta=False, is_maze=True):
    cond = cond.long()
    trajectories = [get_prompt(prompt_trajectories[ind], num_episodes, num_steps, is_meta, is_maze=is_maze) for ind in
                    cond]
    return torch.tensor(np.array(trajectories))
...
class Tasksmeta(nn.Module):
    "Diffloader with a Transformer backbone"
    ...
    def __init__(
        ...
        self.language_prompt_list = language_prompt_list
    ...
    def forward(self, x, time, cond, value, context_mask, x_condition=None, force=False, return_cond=False, flag=False,
                attention_mask=None):
        '''
            x : [ batch x horizon x transition ]  torch.Size([32, 32, 4])
        '''
        cond = cond.long()
        num_steps = self.global_config.train_config.prompt_step
        prompt_type = self.global_config.train_config.prompt_type
        prompt_data = get_prompt_batchs(self.prompt_trajectories, cond, prompt_type=prompt_type, num_episodes=1,
                                        num_steps=num_steps, is_meta=False)
        if prompt_type == 'one-hot':
            prompt_data = prompt_data.to(device=x.device, dtype=torch.float32).reshape(cond.shape[0], 1, -1)
            num_steps = 1
        elif prompt_type == 'language':
            language_prompt_value_list = []
            for cond_item in cond:
                language = self.language_prompt_list[cond_item]
                language_prompt_value_list.append(language[:, :200])
            language_prompt_value_list = torch.tensor(language_prompt_value_list)
            prompt_data = language_prompt_value_list
            prompt_data = prompt_data.to(device=x.device, dtype=torch.float32).reshape(cond.shape[0], 1, -1)
            num_steps = 1
        else:
            prompt_data = prompt_data.to(device=x.device, dtype=torch.float32).reshape(cond.shape[0], -1,
                                                                                   self.state_dim + self.action_dim)  # TODO
        ...
        if not force:
            ...
            mask = 1 if return_cond else 0  # 测试时force为Ture
        batch_size, seq_length = x.shape[0], x.shape[1]
        value = value.view(-1, 1)
        value = self.return_mlp(value)
        cond_return = (value * mask).unsqueeze(1) + 1e-8
        # print("cond_return:", cond_return.shape)
        # cond_return: torch.Size([32, 1, 256])
        # act_embeddings: torch.Size([32, 8, 256])
        act_embeddings = self.embed_act(x)
        # print("act_embeddings:", act_embeddings.shape)
        addition_length = num_steps + 1 + 1 + 2
        ...
        # print("obs_embeddings:", obs_embeddings.shape)
        # print("act_embeddings:", act_embeddings.shape)
        ...
        # stacked_inputs: torch.Size([32, 56, 256])  # 56=1+1+20(prompt_step, 5)+2(history, 2)+32(horizon, 8)
        # stacked_attention_mask: torch.Size([32, 56])
...
class Tasksmaze(nn.Module):
    ...
    def forward(self, x, time, cond, value, context_mask, x_condition=None, force=False, return_cond=False, flag=False,
        ...
        '''
        cond = cond.long()
        prompt_data = torch.tensor([self.prompt_trajectories[ind] for ind in cond]).to(device=x.device,
...
class TasksmetaAug(nn.Module):
    ...
    def forward(self, x, time, cond, x_condition=None, force=False, return_cond=False, flag=False, attention_mask=None):
        ...
        prompt_data = prompt_data.to(device=x.device, dtype=torch.float32).reshape(cond.shape[0], -1,
                                                                                   self.state_dim + self.action_dim)  # TODO
        prompt_embeddings = self.prompt_embed(prompt_data)
        states, actions, rewards, next_states = x[:, :, :self.state_dim], x[:, :,
                                                                          self.state_dim:self.state_dim + self.action_dim], \
                                                x[:, :,
                                                self.state_dim + self.action_dim:self.state_dim + self.action_dim + 1], \
