...
class BatchEpisodes:
	...
	def observations(self):
		...
		# 	print("self._observations:", self._observations.shape)
		if self._observations is None:
			# print("self._observations_list:", len(self._observations_list))
	...
	def costs(self):
		if self._costs is None:
			...
			for i in range(self.batch_size):
				length = len(self._costs_list[i])
				costs[:length, i] = np.stack(self._costs_list[i], axis=0)
			self._costs = torch.from_numpy(costs).to(self.device)
		return self._costs
	...
	def append(self, observations, actions, rewards, batch_ids, cost_arrays):
		for observation, action, reward, batch_id, cost_array in zip(observations, actions, rewards, batch_ids, cost_arrays):
			if batch_id is None:
				continue
			...
			self._costs_list[batch_id].append(cost_array.astype(np.float32))
