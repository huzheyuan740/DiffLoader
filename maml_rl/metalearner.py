...
class MetaLearner:
	...
	def sample(self, tasks, first_order=False, writer=None, current_batch=None):
		"""
		Sample trajectories (before and after the update of the parameters)
		for all the tasks `tasks`.
		"""
		episodes = []
		for task in tasks:  # 这里应该对应不同的MEC环境设置
			self.sampler.reset_task(task)
			# <maml_rl.sampler.BatchSampler>
			train_episodes = self.sampler.sample(self.policy, gamma=self.gamma, device=self.device, writer=writer)

			params = self.adapt(train_episodes, first_order=first_order)
			print("+++++++++++++++++++++++++++++++++++valid")

			valid_episodes = self.sampler.sample(self.policy, params=params, gamma=self.gamma, device=self.device, writer=writer)

			# writer.add_scalar(f'total_rewards/{task}_reward', train_episodes.rewards[-1, 0], current_batch)
			# writer.add_scalar(f'total_rewards/{task}_cost', train_episodes.costs[-1, 0], current_batch)
			# writer.add_scalar(f'total_rewards/{task}_path_length', train_episodes.rewards.shape[0], current_batch)

			show_episodes = train_episodes

			writer.add_scalar(f'total_rewards/{task}_reward', show_episodes.rewards[-1, 0], current_batch)
			writer.add_scalar(f'total_rewards/{task}_cost', show_episodes.costs[-1, 0], current_batch)
			writer.add_scalar(f'total_rewards/{task}_path_length', show_episodes.rewards.shape[0], current_batch)
